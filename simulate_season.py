...
class simulateSeason:
    ...
    def runs_per_game(self):
        ...
        home_rg = (
            gamelogs[gamelogs["date"] > int(season_string + "0717")]
    ...
    def get_season_schedule(self, date_filter=None):
        ...
        if self.pybaseball_schedule:
            ...
            schedule = pd.read_csv(
                "data/schedule/{}SKED.txt".format(season_to_simulate), header=None
            ).iloc[:, 0:10]
        ...
        visiting_games = (
            schedule.groupby("visiting_team")["number_of_games"].count().reset_index()
        )
        visiting_games.columns = ["team", "visiting_number_of_games"]
        home_games = (
            schedule.groupby("home_team")["number_of_games"].count().reset_index()
        )
        home_games.columns = ["team", "home_number_of_games"]
        total_games_by_team = (
            visiting_games.set_index("team")
            .join(home_games.set_index("team"))
            .reset_index()
        )
        total_games_by_team["total_games_played"] = (
            total_games_by_team["home_number_of_games"]
    ...
    def sim_season(self, schedule_rg, distribution="beta"):
        if distribution.lower() == "beta":
            schedule_rg["outcome"] = schedule_rg.apply(
                lambda row: random.betavariate(row["home_mean"], row["home_std"])
                - random.betavariate(row["visiting_mean"], row["visiting_std"]),
                axis=1,
            )
            schedule_rg["win"] = schedule_rg.apply(
                lambda row: str(
                    np.where(row["outcome"] > 0, row["home_team"], row["visiting_team"])
                ),
                axis=1,
            )
            return (
                schedule_rg.groupby("win")["outcome"]
                .count()
                .reset_index()
                .set_index("win")
                .transpose()
            ...
            schedule_rg["outcome"] = schedule_rg.apply(
                lambda row: random.gauss(row["home_mean"], row["home_std"])
                - random.gauss(row["visiting_mean"], row["visiting_std"]),
                axis=1,
            )
            schedule_rg["win"] = schedule_rg.apply(
                lambda row: str(
            ...
            schedule_rg["outcome"] = schedule_rg.apply(
                lambda row: random.lognormvariate(row["home_mean"], row["home_std"])
                - random.lognormvariate(row["visiting_mean"], row["visiting_std"]),
                axis=1,
            )
            schedule_rg["win"] = schedule_rg.apply(
                lambda row: str(
            ...
            schedule_rg["outcome"] = schedule_rg.apply(
                lambda row: random.gammavariate(row["home_mean"], row["home_std"])
                - random.gammavariate(row["visiting_mean"], row["visiting_std"]),
                axis=1,
            )
            schedule_rg["win"] = schedule_rg.apply(
                lambda row: str(
            ...
            schedule_rg["outcome"] = schedule_rg.apply(
                lambda row: random.weibullvariate(row["home_mean"], row["home_std"])
                - random.weibullvariate(row["visiting_mean"], row["visiting_std"]),
                axis=1,
            )
            schedule_rg["win"] = schedule_rg.apply(
                lambda row: str(
            ...
            print(
                "Pick one of the following distributions {}".format(
                    " | ".join(sorted(["beta", "normal", "gamma", "weibull"]))
                )
            )
    ...
    def simulate(self, ntrials=100):
        ...
        schedule_rg = (
            schedule.set_index("visiting_team")
        ...
        for dist in DISTRIBUTIONS:
            ...
            sim_season_df = (
                pd.concat(
                    [
                        self.sim_season(schedule_rg=schedule_rg, distribution=dist)
        ...
        summary_df = (
            merged.set_index("team")
            .join(total_games_by_team.set_index("team"))
            .reset_index()
        )
        ...
        if self.gcp:
            ...
            summary_df.to_html(
                "data/simulations/dev/sim_{season}.html".format(season=self.season + 1),
                index=False,
            )
            summary_df.to_csv(
                "data/simulations/dev/sim_{season}.csv".format(season=self.season + 1),
                index=False,
            )
...
def main(argv=None):
    ...
    parser.add_argument(
        "--season", dest="season", default=int(2020), help="Season you wish to simulate"
    )
    parser.add_argument(
        "--ntrials",
        dest="ntrials",
